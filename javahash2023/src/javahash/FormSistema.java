...
public class FormSistema extends javax.swing.JFrame {
    ...
    //Carregar dados buscas para o arrayBusca //metodo CarregaDadosBusca()
    //diferença é o arquivo, vetor pessoa passa  aser dados,
    //precisa instancia pessoa, como saber se deu certo, da um sout
    //Cria metodo para buscar cada elemnto do array de buca no meuHash
    //semana 11, time hash, tem o codigo so pegar
    //medir o tempo de busca de cada elemento
    //for(String s : aarayBusca)
    //  p = meuHash(s);
    //Medir o tempo de busca de cada elemento
    //contar quantos Cpf existem
    //depois repetir a busca de cada elemetno no LinkedList(minhaLista)
    //Salvar dados no arquivo saida
   ...
   void mostra(){
    ...
    void carregaDadosBusca() {
        String csvFileBusca = "dadosBusca.csv";
        String lineBusca = "";
        try (BufferedReader br = new BufferedReader(new FileReader(csvFileBusca))) {
            while ((lineBusca = br.readLine()) != null) {
                arrayBusca.add(lineBusca);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    void buscarElementosHash() {
        long startTime = System.currentTimeMillis();
        int cpfEncontrados = 0;
        for (String cpf : arrayBusca) {
            Pessoa pessoa = meuHash.get(cpf);
            if (pessoa != null) {
                cpfEncontrados++;
            }
        }
        ...
        System.out.println("Tempo total de busca no hash: " + duracao + " ms");
        System.out.println("Total de CPFs encontrados: " + cpfEncontrados);
}

    
    void buscarElementosLista() {
        long startTime = System.currentTimeMillis();
        int cpfEncontrados = 0;
        for (String cpf : arrayBusca) {
        ...
        long endTime = System.currentTimeMillis();
        long duracao = endTime - startTime;
        System.out.println("Tempo total de busca na lista: " + duracao + " ms");
        System.out.println("Total de CPFs encontrados: " + cpfEncontrados);
}
    ...
    private void btnAddhmActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddhmActionPerformed
        ...
        limpaCampos();
        mostra(); //como percorrer um hashmap e mostrar osdados dele no list interface
    ...
    private void btnCarregaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCarregaActionPerformed
        carregaDados();
        mostra();
        carregaDadosBusca(); //dadosbusca
        salvarDados("dadosSaida.cvs");
        buscarElementosHash();
        buscarElementosLista();
    }//GEN-LAST:event_btnCarregaActionPerformed
    void salvarDados(String nomeArquivoSaida) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(nomeArquivoSaida))) {
            for (String cpf : arrayBusca) {
                ...
                for (Pessoa pessoa : minhaLista) {
                    if (pessoa.getCpf().equals(cpf)) {
                        cpfEncontrado = true;
                        writer.write(cpf + "\t" + pessoa.getNome());
                        writer.newLine();
                        break;
                    }
                }
